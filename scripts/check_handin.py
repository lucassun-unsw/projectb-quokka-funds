...
agent_edited = [
    f for f in ("AGENTS.md", "CLAUDE.md")
    if (ROOT / f).exists()
...
docx = (ROOT / "report" / "report.docx").exists()
pdf = (ROOT / "report" / "report.pdf").exists()
warn(docx or pdf, "report source present (report/report.docx)",
     "no report/report.docx yet - author the report in Word")
# The PDF is what gets submitted, so it needs its OWN reminder. Accepting "docx
# OR pdf" as one check meant a folder holding only the .docx reported clean, and
# the brief asks for report/report.pdf.
warn(pdf, "report/report.pdf present (the submitted file)",
     "no report/report.pdf yet - export it from Word once the fields are updated; "
     "the .docx is the editable source, the PDF is what you hand in")
